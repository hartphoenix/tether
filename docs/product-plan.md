...
Implementation checkpoint (2026-09-03): Phase 4 is complete for exact cmux build `0.64.22 (102) [ddd4a01bc]`. The adapter captures immutable exact-build and window/workspace/surface targets, anchors first placement through `pane.create`, reuses live review panes, applies foreground-only focused fallback, and never substitutes the system browser for cmux errors. Recents lazily creates and reuses one native Dock browser surface, distinguishes daemon instances in its restored URL, and preserves background focus. The detached signed-capability bridge supports daemon callbacks without persistent credentials or broader socket configuration; direct and callback readiness are reported independently. Live validation confirmed document placement, no-focus behavior, callback readiness, first Dock creation, and repeated reuse of the identical Recents surface and session. Automated unit, HTTP, CLI, lifecycle, process-bridge, type, and web-build checks pass.
