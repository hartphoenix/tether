...
| Create browser split        | exact-commit RPC source and live verification                         | `browser.open_split` accepts `show_omnibar: false` and reports whether it split or reused a pane | create review surfaces blank and chromeless; isolate the first if cmux reused a pane     |
| Add browser tab             | exact-commit RPC source                                                | `browser.open_split` can reuse the preferred right sibling while preserving hidden chrome        | reuse a live-discovered review pane; relocate the new surface there if placement differs |
...
| Target source surface       | exact-commit RPC source and live verification                         | `browser.open_split` accepts exact window, workspace, and source-surface UUIDs                   | anchor review placement to the captured invoking surface                                |
| Control focus               | installed CLI help                                                    | creation commands accept `--focus true|false`                                                    | carry focus on each open request                                                        |
| Hide browser chrome         | exact-commit RPC source                                                | workspace `browser.open_split` accepts `show_omnibar: false`; Dock creation does not              | hide chrome on newly opened review surfaces; retain visible Recents chrome until cmux ships Dock support |
...
Post-release cmux `main` already adds Dock-aware `browser.open_split` plus Dock browser `"chrome": false`; the work is also present in cmux Nightly but not in stable 0.64.22. Tether keeps the stable adapter exact-commit-gated until cmux publishes those capabilities in a stable build. [cmux Dock parity commit](https://github.com/manaflow-ai/cmux/commit/11b8615812b6685421f3a8229b7da2b5ecfafad8), [cmux Nightly](https://github.com/manaflow-ai/cmux/releases/tag/nightly)
