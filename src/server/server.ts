...
import { recordRecent, recordRecents, removeRecent } from "../recents/service";
...
export function createDaemon(options: DaemonOptions = {}): TetherDaemon {
  ...
  addButton.addEventListener('click',async()=>{if(busy||selectionMode||!pickerAvailable)return;busy=true;renderControls();try{const result=await post('pick',{});if(!result.cancelled){await refreshFiles();setStatus('Added '+result.added+' file'+(result.added===1?'':'s')+'.')}}catch(error){setStatus(error.message)}finally{busy=false;renderControls()}});
  selectButton.addEventListener('click',()=>{if(selectionMode){selectionMode=false;selected.clear();closeConfirm()}else selectionMode=true;render()});batchRemove.addEventListener('click',()=>openConfirm('remove',batchRemove));batchTrash.addEventListener('click',()=>openConfirm('trash',batchTrash));confirmCancel.addEventListener('click',closeConfirm);confirmAction.addEventListener('click',runBatch);
    ...
    if (pathname.startsWith("/r/")) {
      ...
      if (request.method === "POST" && suffix === "/api/pick") {
        ...
        try {
          ...
          const result = await recordRecents(recents, hostAdapter, paths, session.target);
