...
export const RETIRED_WAVE_WIDGET_IDS = [
  "tether-recent-1", "tether-recent-2", "tether-recent-3",
  "tether-markdown",
...
export type WaveLauncherOptions = { widgetsPath?: string; mdreviewPath?: string; runtimePath?: string; recents?: RecentEntry[]; installationPaths?: string[] };
...
function tetherWidgets(mdreviewPath: string, runtimePath: string): Widgets {
  return {
    "tether-recents": commandWidget("Tether Folio", "Open Tether Folio", "regular@file-lines", ["folio"], 1090, mdreviewPath, runtimePath),
  };
}

/** Detect installation without launching Wave or reading its private configuration. */
export async function waveInstallationDetected(options: WaveLauncherOptions = {}): Promise<boolean> {
  const overridden = options.widgetsPath !== undefined || process.env.WAVETERM_CONFIG_DIR !== undefined;
  if (!overridden && (process.env.WAVETERM === "1" || process.env.TERM_PROGRAM === "waveterm")) return true;
  const candidates = options.installationPaths ?? (overridden ? [] : [
    "/Applications/Wave.app", join(homedir(), "Applications/Wave.app"),
    join(homedir(), "Library/Application Support/waveterm/bin/wsh"),
  ]);
  for (const path of [dirname(paths(options).widgetsPath), ...candidates]) {
    try { if (await stat(path)) return true; }
...
export async function waveLauncherStatus(options: WaveLauncherOptions = {}) {
  ...
  const installed = [...WAVE_WIDGET_IDS, ...RETIRED_WAVE_WIDGET_IDS].filter((id) => Object.hasOwn(widgets, id));
...
export async function installWaveLaunchers(options: WaveLauncherOptions = {}) {
  ...
  Object.assign(widgets, tetherWidgets(resolved.mdreviewPath, resolved.runtimePath));
